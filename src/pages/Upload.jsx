...
import { Upload, Lock, CheckCircle, Loader2, Image, LogOut, Eye, EyeOff, AlertTriangle, X } from 'lucide-react';
  ...
  return new Promise((resolve, reject) => {
    ...
    xhr.onload = () => {
      if (xhr.status === 200) {
        ...
        // Save to localStorage so it appears immediately in local gallery
        saveLocalPhoto({
...
function UploadCard({ file, status, progress }) {
  ...
  return (
    <div className="flex items-center gap-4 bg-gray-50 border border-gray-200 rounded-xl p-3 text-dark">
      <img src={url} alt={file.name} className="w-14 h-14 rounded-lg object-cover shrink-0" />
      <div className="flex-1 min-w-0">
        <p className="font-poppins font-medium text-sm truncate">{file.name}</p>
...
export default function UploadPage() {
  ...
  if (!loggedIn) {
    return (
      <div className="min-h-screen bg-dark flex items-center justify-center px-4">
        <SEO
          title="Admin Upload | Speed Track Towing"
          description="Secure portal to upload photos to the Speed Track Towing gallery."
        />
        <motion.div
        ...
        >
          <div className="text-center mb-8">
            ...
            <p className="text-gray-500 text-sm font-poppins">Enter your admin password to upload gallery photos.</p>
          </div>
  ...
  return (
    <div className="min-h-screen bg-gray-50 pt-36 pb-16">
      <SEO
        title="Upload Gallery Photos | Speed Track Towing"
        description="Portal to upload photos to the Speed Track Towing gallery."
      />
      <div className="max-w-3xl mx-auto px-4 sm:px-6">
        {/* Header */}
        <div className="flex items-center justify-between mb-10 text-dark">
          <div>
            <h1 className="font-barlow font-extrabold text-4xl uppercase tracking-tight">
              Upload <span className="text-primary">Photos</span>
            </h1>
            <p className="text-gray-500 font-poppins text-sm mt-1">Upload photos to show in the gallery page</p>
          </div>
          <button
        ...
        {uploads.length > 0 && (
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
            ...
            {uploads.some(u => u.status === 'done') && (
              <div className="mt-5 p-4 bg-[#16a34a]/8 border border-[#16a34a]/20 rounded-xl text-dark">
                <p className="text-[#16a34a] font-poppins text-sm font-semibold">
                  ✓ Photos uploaded! They are now live on the public Gallery page.
                </p>
              </div>
            )}
          </div>
